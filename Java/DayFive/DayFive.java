...
class DayFive {
    ...
    public void computer(int inputInstruction) {
		...
		while (i<inputArr.length) {
            ...
            int parOneMode = instString.length() >= 3 ? Character.getNumericValue(instString.charAt(instString.length()-3)) : 0 ;
            int parTwoMode = instString.length() >= 4 ? Character.getNumericValue(instString.charAt(instString.length()-4)) : 0 ;
            parOne = (parOneMode == 1 ? inputArr[i+1] : inputArr[inputArr[i+1]]);
            if(opCode != 4 && opCode != 3) {
                parTwo = (parTwoMode == 1 ? inputArr[i+2] : inputArr[inputArr[i+2]]);
            }
            

            if(opCode == 3) {

                inputArr[inputArr[i+1]] = inputInstruction;
                i+=2;
                
            } else if (opCode == 4) { // output instruction

                System.out.println("output: "+ inputArr[inputArr[i+1]]);
                i+=2;
         
            } else {

            if (opCode == 1) inputArr[inputArr[i+3]] = parOne + parTwo;
             else if (opCode == 2) inputArr[inputArr[i+3]] = parOne * parTwo;
             i+=4;
